...
const auth = new google.auth.GoogleAuth({
    keyFile: path.join(__dirname, '/etc/secrets/credentialsDrive.json'), // Ruta al archivo JSON de credenciales
...
const getOrCreateFolder = async (folderName) => {
    try {
        ...
        if (response.data.files.length > 0) {
            ...
            console.log(`Carpeta encontrada: ${response.data.files[0].name} (ID: ${response.data.files[0].id})`);
