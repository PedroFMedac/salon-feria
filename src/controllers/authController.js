...
const login = async (req, res) => {
    ...
    try {
        ...
        const userDoc = !nameQuery.empty ? nameQuery.docs[0] : !emailQuery.empty ? emailQuery.docs[0] : null;
        ...
        const token = jwt.sign(
            { id: userDoc.id, name: user.name, email: user.email, rol: user.rol, designComplete: user.design, information: user.information },
        ...
        return res.status(500).send({ error: 'internal_server_error', message: 'Server error occurred' });
    }
};
