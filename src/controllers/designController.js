...
const createDesign = async (req, res) => {
    ...
    if (!standID || !modelID) {
        return res.status(400).json({ message: 'Please provide standID and modelID' });
    }
    ...
    try {
        ...
        // Respuesta exitosa
        res.status(200).json({ message: 'Design created successfully', idDesign: designRef.id });
...
const getDesign = async (req, res) => {
    ...
    try {
        ...
        const designSnapshot = await db.collection('design').where('companyID', '==', companyID).get();

        if (designSnapshot.empty) {
            return res.status(404).json({ message: 'Design not found for this company.' });
        }

        // Obtener los datos del diseño
        const designData = designSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))[0];
        const { standID, modelID, fileID, createdAt, ...filtereddesignData } = designData;
        ...
        const fileData = fileSnapshot.exists ? { id: fileSnapshot.id, ...fileSnapshot.data() } : null;
...
const updateDesign = async (req, res) => {
    ...
    try {
        ...
        const existDesignSnapshot = await db.collection('design').where('companyID', '==', companyID).get();
        if (existDesignSnapshot.empty) {
            return res.status(400).json({ message: 'Design does not exist for this company.' });
        }

        // Verificar si existen archivos asociados a esta compañía
        const existFileSnapshot = await db.collection('files').where('companyID', '==', companyID).get();
        if (existFileSnapshot.empty) {
            return res.status(400).json({ message: 'Files do not exist for this company.' });
        }
...
const getAllStands = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error getting stands', error: error.message });
    }
};
...
module.exports = { createDesign, getDesign, getAllDesigns, updateDesign, deleterDesign, getAllModels, getAllStands };